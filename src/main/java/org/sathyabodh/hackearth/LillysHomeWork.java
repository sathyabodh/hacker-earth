...
class LillysHomeWork {

//    public static int lilysHomework(List<Integer> arr) {
//        Integer[] array = arr.toArray (new Integer[1]);
//        return minmumSwaps (array);
//        // return mergeSort(array, 0, array.length-1);
//    }
//
//    public static int minmumSwaps(Integer[] arr) {
//        Integer[] firstArray = Arrays.copyOfRange (arr, 0, arr.length);
//        Integer[] secondArray = Arrays.copyOfRange (arr, 0, arr.length);
//        int swaps1 = minmumSwaps (firstArray, (first, second) -> first - second);
//        int swaps2 = minmumSwaps (secondArray, (first, second) -> second - first);
//        System.out.println ("Swaps " + swaps1 + " : " + swaps2);
//        return Math.min (swaps1, swaps2);
//    }
//
//    private static int minmumSwaps(Integer[] arr, Comparator<Integer> comp) {
//        Map<Integer, Integer> valueToIndexMap = new LinkedHashMap<> ();
//        Integer[] temp = Arrays.copyOfRange (arr, 0, arr.length);
...
//                int currentIndex = i;
//                int value = arr[i];
//                int origIndex = valueToIndexMap.get (temp[i]);
//                swap (arr, i, origIndex);
//                valueToIndexMap.put (value, origIndex);
...
//    }
//
    public static int swaps(long[] unsortedValues) {
        int swaps = 0;
...
//    }
    ...
    public static int minmumSwaps(long[] arr){
//        Integer[] firstArray = Arrays.copyOfRange(arr, 0, arr.length);
//        Integer[] secondArray = Arrays.copyOfRange(arr, 0, arr.length);
//        int swaps1 = minmumSwaps(firstArray, false);
//        int swaps2 = minmumSwaps(secondArray, true);
        int swaps2 = 0;
        int swaps1 = minmumSwaps(arr, false);
        System.out.println ("M:" + swaps1 + " M2:" + swaps2);
        return Math.min(swaps1, swaps2);
    }
    private static int minmumSwaps(long[] arr, boolean isReverse){
...
//        }
        ...
        for(int i = 0; i < arr.length; ++i){
            if(arr[i] != temp[i]){
                ...
                int currentIndex = i;
                long value = arr[i];
                Integer origIndex = valueToIndexMap.get(temp[i]);
                swap(arr, i, origIndex);
                valueToIndexMap.put(value, origIndex);
                // valueToIndexMap.put(temp[i], i);
            }
    ...
    private static void swap(long[] a, Integer i , Integer j){
        long temp = a[i];
        a[i] = a[j];
        a[j] = temp;
    }
...
//        Integer [] a = {3,7,15,12};
        int n = a.length;
        // Output will be 5
        System.out.println (new LillysHomeWork ().minmumSwaps (a));
        Collections.sort (new ArrayList<Integer> (), (first, second)-> first-second);
//        System.out.println (new LillysHomeWork ().swaps (a));
    }
}
